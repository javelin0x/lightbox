...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    if (hovered)
        window->DrawList->AddRectFilled(center + ImVec2(-8.0f, -8.0f),
            center + ImVec2(+8.0f, +8.0f), ImGui::ColorConvertFloat4ToU32(ImVec4(0.43f, 0.48f, 1.0f, 1.0f)), 2.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(0.0f, 0.0f);
    ...
    else if (*v)
    {
        ...
        // Render a filled square instead of a checkmark
        window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, ImColor(53, 230, 223));
    }

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 1.0f, check_bb.Min.y + style.FramePadding.y - label_size.y / 3.0f);
    if (g.LogEnabled)
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y / 2.0f + style.FramePadding.y));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
